# aug12 plot/data ledger

Every result is plot-ready JSON/CSV with its schema named in the corresponding
`result.md`. A figure remains pending until correctness, negative controls,
soak, timing, and executor-label gates are all satisfied.

| study figure | data artifact | experiment | status |
|---|---|---|---|
...
| Workload proxies | `exp_06_workload_tournament/workload_regimes.json` | Stage 6 | pending |
| Synthetic MoE validation | `exp_06_workload_tournament/moe_policy_tournament.json` | Stage 6 | pending |
| Held-out policy confirmation | `exp_07_policy_confirmation/policy_confirmation.json` | Stage 7 | pending |
