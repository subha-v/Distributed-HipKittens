...
- Stage 0/1 neutral transport plane: **IN PROGRESS** — one-process 64 KiB
  anchor gated; rank-per-GPU CU-push is protocol-correct but exposed an invalid
  cross-CU `clock64()` timer check. Source rev 3 switches stamps to global
  `s_memrealtime`; corrected gate pending.
