# exp_21 — mode 7: direct remote bf16 accumulation in the M7 epilogue (A11/M11)
...
- **F1 (fabric-atomic rate).** If mode 7 lands ABOVE mode 2's 6,866.1 µs at
...
- **F2 (mori heap grain).** If the dual-write detector (cfg bit 34) or the
  ...
  blocker 1b is LIVE and mode 7 cannot ship; that is a hardware finding worth
  publishing (documented UB biting in practice).
- **Success:** mode 7 at best C < mode 2's 6,866.1 through the full gate
...
2. `moe_mps_adapter.cuh`: `mode_is_stream += {7}`; `mode_is_remote_accum`;
   validation (mode 7 ⇒ g==1, pull_fallback==0, C>0); cfg bit 34 = `detect`
...
4. `k0pf6gm_device_tile_mps.hip`: SRC_REV 17; entry guard (mode 7 ⇒ MAXTOK
   power of two); `k0p6_mps_task_done` enqueues at system scope for mode 7;
...
## What mode 7 deliberately does NOT change
...
- Task-done's all-lane `vmcnt(0)` + `__syncthreads()` + tid0 release chain —
  only the release/acquire *scopes* strengthen (agent → system) for mode 7.
