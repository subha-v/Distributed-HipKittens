...
| Fig 3 | Q4 per-layer resource timeline | `aug11/exp_22_timeline/events_b0_reference.json`, `b0_kernel_map.json`, `parity.json`, `tick_rate.json`, `timeline_bins.csv` | exp_22 | **arm (a) LANDED**: reference GEMM+RCCL on shape 5 is three **strictly serialized** intervals, zero overlap — 243.2 µs GEMM / 43.4 µs bias / **241.3 µs RCCL = 45.7% comm share** of 527.9 µs; zero unclassified kernel names. Parity gate passed. **Arm (b) built, queued for the lease** — integral residuals and credit-wait visibility still owed. Arm (c) judged obtainable, not yet requested |
