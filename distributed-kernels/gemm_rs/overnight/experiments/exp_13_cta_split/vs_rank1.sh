...
#
# Copied verbatim from exp_05_release_granularity/vs_rank1.sh with D redirected
# here and the provenance line changed from RELEASE_GROUP to the reducer-split
# column, which is what exp_13 changes. Nothing else differs -- the instrument
# must stay byte-comparable to the run that produced the 353.59/306.23 pair.
#
...
echo "=== exp_13 vs rank-1 [$TAG], start $(date -Is) shapes=$SHAPES iters=$ITERS reps=$REPS"
echo "--- reducer-split column in the source that built it ---"
